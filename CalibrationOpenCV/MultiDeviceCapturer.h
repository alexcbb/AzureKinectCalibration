...
public:
    ...
    */
    MultiDeviceCapturer(const std::vector<uint32_t>& device_indices, int32_t color_exposure_usec, int32_t powerline_freq)
    {
        ...
        // We open each devices set their color control and store them
        for (uint32_t i : device_indices)
        {
            try {
                // construct a device using this index
                k4a::device next_device = k4a::device::open(i);
    ...
    /**
    * Function that starts all the devices given the configurations
    * Note: configs[0] should be the master, the rest subordinate
    *
    ...
    */
        ...
        bool compare_sub_depth_instead_of_color = false)
    {
        ...
        */
        ...
        /*
        ===================
        GET THE MASTER / SUBORDINATE DEVICE CAPTURE
        ===================
        */
        try {
        ...
        */
        ...
        while (!have_synced_images)
        {
            ...
            {
                k4a::image sub_image;
                // We get the image (color or depth) given by the subordinate device
    ...
    */
    ...
    /**
    * Returns one of the subordinate device
    *
    ...
    */
    const k4a::device& getSubordinateDeviceByIndex(size_t i) const
    {
        ...
        {
            std::cerr << "Subordinate index too large! Index given " << i << std::endl;
            exit(1);
        }
